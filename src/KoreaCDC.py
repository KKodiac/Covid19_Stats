...
class KoreaCovid:
    def __init__(self):
        ...
        self.data_korea = f'./data/Korea/covid_dat_kr_total.csv'
        self.date = ctime(time())


    def is_updated(self) -> bool:
        with open(self.data_korea) as file:
    ...
    def __append_header(self):
        with open(self.data_korea, 'w+') as file:
                df = csv.writer(file)
                df.writerow(keys)
                file.close()
    ...
    def get_country_data(self, dt):
        ...
        if(path.isfile(self.data_korea)==False):
            self.__append_header()

        with open(self.data_korea, 'a+') as file:
            df = csv.writer(file)
            df.writerow(status)
            file.close()
